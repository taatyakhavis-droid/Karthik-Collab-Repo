    ...
    <div className="min-h-screen bg-[#0e0e0e] text-white overflow-x-hidden">
      ...
      <header className="fixed top-0 flex justify-between items-center w-full px-6 py-4 bg-[#0e0e0e]/80 backdrop-blur-xl z-50 shadow-[0px_20px_40px_rgba(0,0,0,0.4)]">
        ...
        <nav className="hidden md:flex gap-8">
          <Link className={`font-archivo tracking-tighter uppercase text-xs ${!activeFilter ? 'text-[#FBDE06]' : 'text-gray-400 hover:text-[#FBDE06]'} transition-colors`} to="/" onClick={clearFilter}>Home</Link>
          {isAdmin && <Link className="font-archivo tracking-tighter uppercase text-xs text-gray-400 hover:text-[#FBDE06] transition-colors" to="/admin">Admin</Link>}
        </nav>
      </header>
      ...
      <main className="pt-28 pb-32 px-6 max-w-7xl mx-auto">
        {activeFilter && (
          <div className="mb-8 flex flex-wrap gap-2 animate-in slide-in-from-left duration-700">
            <button onClick={clearFilter} className="bg-[#FBDE06] text-[#0e0e0e] px-4 py-1.5 rounded-full font-black uppercase text-[10px] tracking-tighter flex items-center gap-2">
              {activeFilter} <span className="material-symbols-outlined text-[14px]">close</span>
            </button>
          </div>
        )}
        ...
        {loading ? (
            ...
            <section className="mb-20 relative">
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-7 rounded-3xl overflow-hidden neumorphic-flat h-full min-h-[300px]">
                  <img alt={featuredPost.title} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" src={featuredPost.cover_image} style={{ objectPosition: featuredPost.image_position }} />
                </div>
                <div className="lg:col-span-5 flex flex-col gap-4">
                  <span className="text-[#FBDE06] font-bold tracking-[0.2em] uppercase text-[10px]">{activeFilter ? activeFilter : 'Featured Story'}</span>
                  <h2 className="font-archivo tracking-[-0.04em] leading-[0.9] text-white" style={{ fontSize: 'clamp(36px, 5vw, 64px)' }}>{featuredPost.title}</h2>
                  <p className="text-[#adaaaa] line-clamp-3 text-lg">{featuredPost.short_description}</p>
                  <div className="flex flex-wrap gap-2">
                    {featuredPost.tags?.map(tag => (
                      <span key={tag} className="text-[#FBDE06]/60 text-[10px] font-bold uppercase tracking-widest border border-[#FBDE06]/20 px-2 py-1 rounded-full">#{tag}</span>
                    ))}
                  </div>
                  <div className="mt-4">
                    <Link to={`/blog/${featuredPost.slug}`} className="bg-[#20201f] text-[#FBDE06] px-8 py-4 rounded-xl neumorphic-flat active:shadow-[inset_4px_4px_8px_#0a0a0a,inset_-4px_-4px_8px_#1a1a1a] transition-all font-bold uppercase tracking-widest inline-flex items-center gap-2 text-xs">
                ...
                <Link key={post.id} to={`/blog/${post.slug}`} className="flex flex-col gap-6">
                  <div className="bg-[#20201f] p-4 rounded-[2rem] neumorphic-flat group cursor-pointer transition-all hover:-translate-y-2">
                    <div className="rounded-2xl overflow-hidden mb-6 relative">
                      <img alt={post.title} className="w-full aspect-[4/3] object-cover scale-105 group-hover:scale-100 transition-transform duration-500" src={post.cover_image} style={{ objectPosition: post.image_position }} />
                      <div className="absolute top-4 left-4 bg-[#FBDE06] text-[#0e0e0e] font-black px-3 py-1 rounded-full uppercase tracking-tighter text-[10px]">{post.category}</div>
                    </div>
                    <div className="px-2 pb-4">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-[#adaaaa] font-bold tracking-widest uppercase text-xs">{new Date(post.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase()}</span>
                        <span className="text-[#adaaaa] text-[10px]">{post.read_time}</span>
                      </div>
                      <h3 className="font-archivo leading-tight group-hover:text-[#FBDE06] transition-colors tracking-tight text-xl mb-2">{post.title}</h3>
          ...
          <div className="text-center py-20">
            ...
            <button onClick={clearFilter} className="text-[#FBDE06] mt-4 uppercase font-bold tracking-widest text-xs">Clear Filter</button>
          </div>
        )}
      </main>
      ...
      <div className={`fixed inset-0 z-[100] transition-all duration-700 ${isMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute top-0 left-0 h-full w-full md:w-[480px] bg-[#1a1a1a] shadow-[20px_0_100px_rgba(0,0,0,0.8)] p-12 flex flex-col transition-transform duration-700 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-10">
            {categories.length > 0 && (
              <section>
                <label className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-500 mb-4 block">Filter by Category</label>
                <div className="grid grid-cols-2 gap-3">
                  {categories.map(cat => (
                    <button key={cat} onClick={() => selectCategory(cat)} className={`px-4 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all text-left ${selectedCategory === cat ? 'bg-[#FBDE06] text-[#0e0e0e] shadow-xl' : 'text-gray-400 neumorphic-flat hover:text-white'}`}>
                      {cat}
                    </button>
                  ))}
                </div>
            ...
            {tags.length > 0 && (
              <section>
                <label className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-500 mb-4 block">Filter by Tag</label>
                <div className="flex flex-wrap gap-2">
                  {tags.map(tag => (
                    <button key={tag} onClick={() => selectTag(tag)} className={`px-3 py-2 rounded-full text-[11px] font-bold uppercase tracking-widest transition-all ${selectedTag === tag ? 'bg-[#FBDE06] text-[#0e0e0e]' : 'text-gray-400 neumorphic-flat hover:text-white'}`}>
                      #{tag}
                    </button>
                  ))}
                </div>
            ...
            <nav className="flex flex-col gap-6">
              <Link onClick={() => { setIsMenuOpen(false); clearFilter(); }} to="/" className="text-4xl font-archivo text-white hover:text-[#FBDE06] transition-colors duration-300">HOME</Link>
              {isAdmin && (
          ...
          <div className="pt-12 border-t border-[#262626] flex items-center justify-between">
            <div className="flex gap-6">
              <a href="#" className="text-gray-500 hover:text-[#FBDE06] transition-colors"><i className="fa-brands fa-instagram text-xl" /></a>
              <a href="#" className="text-gray-500 hover:text-[#FBDE06] transition-colors"><i className="fa-brands fa-x-twitter text-xl" /></a>
            </div>
