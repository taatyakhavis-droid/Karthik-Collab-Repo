      ...
      <header className="fixed top-0 flex justify-between items-center w-full px-6 py-4 bg-[#0e0e0e]/80 backdrop-blur-xl z-50 shadow-[0px_20px_40px_rgba(0,0,0,0.4)]">
        ...
        <div className="flex items-center gap-6">
          <nav className="hidden md:flex gap-6 items-center">
            <Link to="/" className="text-gray-400 font-bold hover:text-[#FBDE06] transition-colors text-xs uppercase tracking-widest">Live Site</Link>
            <Link to="/admin/categories" className="text-gray-400 font-bold hover:text-[#FBDE06] transition-colors text-xs uppercase tracking-widest">Categories & Tags</Link>
          ...
          <Link to="/admin/new" className="bg-[#FBDE06] text-[#0e0e0e] px-6 py-2.5 rounded-xl font-black uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition-transform flex items-center gap-2">
      ...
      <main className="pt-28 pb-20 px-6 max-w-7xl mx-auto">
        ...
        <section className="neumorphic-flat rounded-[2rem] overflow-hidden">
          <div className="flex items-center justify-between px-8 py-6 border-b border-[#262626]">
            ...
            <Link to="/admin/new" className="text-[#FBDE06] text-xs font-bold uppercase tracking-widest hover:scale-105 transition-transform flex items-center gap-1">
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <Link to="/admin/new" className="text-[#FBDE06] mt-4 inline-block text-xs font-bold uppercase tracking-widest">Create your first post →</Link>
            </div>
          ) : (
              ...
              {posts.map(post => (
                <div key={post.id} className="flex items-center gap-4 px-8 py-5 hover:bg-[#1a1a1a] transition-colors group">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <Link to={`/admin/edit/${post.slug}`} className="w-9 h-9 rounded-lg neumorphic-flat flex items-center justify-center text-gray-500 hover:text-[#FBDE06] transition-colors">
                      <span className="material-symbols-outlined text-sm">edit</span>
                    </Link>
                    <Link to={`/blog/${post.slug}`} target="_blank" className="w-9 h-9 rounded-lg neumorphic-flat flex items-center justify-center text-gray-500 hover:text-[#FBDE06] transition-colors">
        ...
        <nav className="md:hidden fixed bottom-6 left-0 right-0 z-50 flex justify-around items-center max-w-md mx-auto bg-[#20201f]/60 backdrop-blur-2xl rounded-[2rem] px-8 py-3 shadow-[5px_5px_10px_#0a0a0a,-5px_-5px_10px_#1a1a1a] w-[90%]">
          <Link to="/" className="text-gray-500 p-3"><span className="material-symbols-outlined">home</span></Link>
          <Link to="/admin/new" className="text-gray-500 p-3"><span className="material-symbols-outlined">add_circle</span></Link>
          <Link to="/admin/categories" className="text-gray-500 p-3"><span className="material-symbols-outlined">label</span></Link>
