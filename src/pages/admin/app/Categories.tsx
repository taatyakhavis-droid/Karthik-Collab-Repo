...
export default function Categories() {
  ...
  const handleDeleteCategory = async (name: string) => {
    if (!window.confirm(`Delete category "${name}"? This won't change existing posts.`)) return;
  ...
  const handleDeleteTag = async (name: string) => {
    if (!window.confirm(`Delete tag "${name}"? This won't change existing posts.`)) return;
    await postService.deleteTag(name);
    await fetchAll();
  };

  const Panel = ({ title, items, newValue, onNewChange, onAdd, onDelete, addLoading, error, placeholder, isTag }: {
    title: string; items: string[]; newValue: string; onNewChange: (v: string) => void;
    onAdd: () => void; onDelete: (n: string) => void; addLoading: boolean; error: string;
    placeholder: string; isTag?: boolean;
  }) => (
    <section className="neumorphic-flat p-8 rounded-[2rem]">
      <h3 className="font-archivo uppercase tracking-tight text-lg mb-6 flex items-center gap-3">
        ...
        {title}
        <span className="text-gray-600 text-sm font-normal normal-case tracking-normal ml-auto">{items.length} {items.length === 1 ? 'item' : 'items'}</span>
      </h3>

      <div className="space-y-3 mb-6 max-h-[300px] overflow-y-auto pr-1">
        {items.length === 0 && (
          <p className="text-gray-600 text-xs uppercase tracking-widest text-center py-6">No {title.toLowerCase()} yet.</p>
        )}
        {items.map(item => (
          <div key={item} className="flex items-center justify-between p-4 neumorphic-inset rounded-xl">
            ...
            <button onClick={() => onDelete(item)} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-500 hover:text-[#ff7351] transition-colors">
      ...
      <div className="border-t border-[#1a1a1a] pt-6 space-y-3">
        ...
        <div className="flex gap-2">
          <input
            className="flex-1 bg-transparent neumorphic-inset rounded-xl px-4 py-3 border-0 focus:ring-1 focus:ring-[#FBDE06]/40 text-white placeholder:text-gray-600 text-sm"
          ...
          />
          <button
            onClick={onAdd} disabled={addLoading || !newValue.trim()}
  ...
  return (
    <div className="min-h-screen bg-[#0e0e0e] text-white">
      ...
      <header className="fixed top-0 flex justify-between items-center w-full px-6 py-4 bg-[#0e0e0e]/80 backdrop-blur-xl z-50 shadow-[0px_20px_40px_rgba(0,0,0,0.4)]">
        ...
        <button onClick={() => authService.logout()} className="text-gray-400 hover:text-red-400 transition-colors text-xs uppercase tracking-widest font-bold">Logout</button>
      </header>
